/**
 * clientReady.js — Discord 'clientReady' event
 *
 * Fires when the bot shard is fully connected and operational.
 * Named 'clientReady.js' so the event loader binds it to the correct
 * 'clientReady' event (Discord.js v14 renamed 'ready' → 'clientReady').
 *
...
module.exports = async (client) => {
  try {
    ...
    const updatePresence = () => {
      const userDisplay  = totalUsers > 1000 ? `${(totalUsers / 1000).toFixed(1)}K` : totalUsers;
      const guildDisplay = guilds     > 1000 ? `${(guilds     / 1000).toFixed(1)}K` : guilds;
    ...
    Logger.error('clientReady', 'Error in ready event handler', e);
  }
};
